...
\eyebrow{como as peças se encaixam}
...
\begin{enumerate}[leftmargin=1.4em,itemsep=4pt]
  \item \textbf{O banco de dados é a fonte da verdade para o pareamento.} Uma imagem
        nunca carrega seu vínculo no nome do arquivo. O fluxo é sempre: selecionar ou
        criar a carcaça, depois capturar/importar \emph{dentro} do contexto dessa
        carcaça, de modo que a imagem seja gravada com seu \tele{carcass\_id} já
        definido. A reconstrução pela ordem dos arquivos é estruturalmente
        impossível.
  \item \textbf{As fontes de captura são intercambiáveis.} A captura por webcam roda
        no front end via a API de câmera do navegador; o Kinect (depth) roda através
        de um sidecar Python; ambos são apenas fontes de uma imagem pareada.
  \item \textbf{As medições vêm com honestidade sobre seu status.} A cor codifica o
