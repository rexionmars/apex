...
\eyebrow{como ler o instrumento}

\vspace{4pt}
O Apex segue a anatomia do software de controle de missão. A janela é organizada em
regiões fixas, e você trabalha \textbf{selecionando um objeto} na árvore de navegação
e \textbf{escolhendo uma vista} dele.
...
\begin{note}[title={O Inspetor sempre acompanha a sua seleção}]
...
Ele é somente leitura; a edição acontece nas vistas.
\end{note}

A barra de status na parte inferior é o pulso do instrumento: ela mostra se o
\tele{engine} de inferência está \pillok{ready} e em qual \tele{device} (por exemplo,
\tele{mps}), além dos totais correntes do conjunto de dados.
