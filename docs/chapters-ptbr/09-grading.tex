...
\eyebrow{escores independentes e cegos com concordância}

\vspace{4pt}
A visão de \textbf{Avaliação} de um lote reúne graus de múltiplos avaliadores, cada
um pontuando de forma independente e cega, e mede o grau de concordância entre eles.
O resultado é uma referência humana: um grau atribuído \emph{sobre a imagem},
acompanhado de uma medida de sua confiabilidade.

\section{Como funciona uma sessão}

\begin{enumerate}[leftmargin=1.4em,itemsep=3pt]
  \item Um \textbf{avaliador} é adicionado ou selecionado.
  \item Uma \textbf{sessão cega} é iniciada no lote: os escores dos demais avaliadores
        ficam ocultos, de modo que cada avaliação seja independente.
  \item Para cada carcaça com imagem, dois eixos são pontuados em uma escala de 1--5:
...
é sinalizado. Essa medida permite avaliar a confiabilidade de um grau antes de utilizá-lo
como referência.
...
parcialmente. A interface registra isso: a validação preditiva completa do acabamento depende
