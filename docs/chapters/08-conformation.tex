...
inward (concave). Apex implements this rule as a measurement.
...
differentiation, so it is robust to mask noise, and yields a value:
...
The map is consistent with anatomy without any label: the legs are blue, the
notch between them is red, the loin is near-neutral. This correspondence is
verifiable by eye.
...
size (Station~13). The grade should be treated as an indicative reference, to be
validated as the paired dataset grows.
\end{caution}
