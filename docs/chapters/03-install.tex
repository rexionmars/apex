...
\eyebrow{getting the instrument on}

\section{Option A: a pre-built release}

The quickest path is to download a native build for your platform from the
project's \textbf{Releases} page. Each release ships a self-contained archive per
operating system, with the model weights already bundled and no build tools
required:
...
Unpack it and run the app. The Python analysis features still need a Python
environment with the scientific libraries (see below); everything else works out
of the box. Releases are produced automatically by continuous integration
(Station~14).

\begin{note}[title={Building from source}]
If you want to develop, modify, or build for a platform not in the releases,
follow Options~B and~C below.
\end{note}
...
If your system \tele{python3} does not have the scientific libraries, point the
app at one that does:
